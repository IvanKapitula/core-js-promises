...
function getAllResult(promises) {
  return Promise.allSettled(promises).then((finish) => {
    return finish.map((finishItem) => {
      return finishItem.status === 'rejected' ? null : finishItem.value;
    });
  });
}
...
function queuPromises(promises) {
  return promises.reduce(function hi(pie, prom) {
    return pie.then((acc) => {
      return prom.then((val) => acc + val.toString());
    });
  }, Promise.resolve(''));
}
